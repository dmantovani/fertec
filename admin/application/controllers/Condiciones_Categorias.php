...
class Condiciones_Categorias extends CI_Controller {
	...
	public function index()
	{
		...
		$info =  $this->page_model->get_condiciones_categorias();
		$data['info']=$info;
	    ...
	    $this->template->write_view('content', 'layout/condiciones_categorias/list', $data, TRUE); 
	...
	public function add(){
		...
		$this->template->write_view('content', 'layout/condiciones_categorias/add', $data, TRUE); 
		$this->template->render();
	}
	...
	public function save(){
		if (isset($this->session->userdata['logged_in'])) {
			$data = array(
				'categoria' => $_POST['item']
			);
			$this->page_model->insert_condiciones_categorias($data);
			redirect('condiciones_categorias/');
		}else{
			redirect('login/');
		}
	...
	public function update(){
		if (isset($this->session->userdata['logged_in'])) {
			...
			$this->page_model->update_condiciones_categoria($data);
			redirect('condiciones_categorias/');
		}else{
			redirect('login/');
		}
	...
	public function remove(){
		if (isset($this->session->userdata['logged_in'])) {
			$this->page_model->remove_condiciones_categorias();
			redirect('condiciones_categorias/');
		}else{
			redirect('login/');
		}
	...
	public function edit(){
		...
		$CI =& get_instance();
		$info =  $this->page_model->get_condiciones_categorias_id($this->uri->segment(3));
		$data['info']=$info;
		
		$this->template->write_view('content', 'layout/condiciones_categorias/edit', $data, TRUE); 
		$this->template->render();
	}
