...
class Home extends CI_Controller {
	...
	{	
		...
		// Save utm
		// --
	    if(isset($_GET["utm_medium"]) && strlen($_GET["utm_medium"]) > 1) { $this->session->set_userdata("utm_medium",$_GET["utm_medium"]);	}
	    if(isset($_GET["utm_source"])  && strlen($_GET["utm_source"]) > 1) { $this->session->set_userdata("utm_source",$_GET["utm_source"]);	    }
		
		//añadimos los archivos js que necesitemoa		
		//$this->template->add_js('asset/js/home.js');
	    
		//desde aquí también podemos setear el título
		$this->template->write('title', 'Fertec - Login', TRUE);
		...
		//$this->template->write_view('header', 'layout/', $data);
...
//		    $this->email->cc('landbertoldi@bertoldi.com.ar');
	...
	public function cambiar_estado()
	{
		...
		$this->db->where('id', $_POST['presupuesto_id']);
