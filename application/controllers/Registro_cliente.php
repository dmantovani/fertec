...
class Registro_cliente extends CI_Controller {
	...
	{	
		...
		*/
		...
		$this->template->write('title', 'Fertec - Registro del cliente', TRUE);
		...
		$data["provincias"] = $this->page_model->getProvinciasSelect();
 
		$this->template->write_view('content', 'layout/registro/cliente', $data);
	...
	function errorCuit()
	{
		print "<script>alert('El cuit está siendo presupuestado por otro distribuidor'); window.location='".base_url()."registro_cliente'</script>";
		exit;
	}
	
	// ---	
	// Getting provincias via ajax
	// ---
	public function provincias()
	{
		$data["ciudades"] = $this->page_model->getCiudadesSelect($_GET["provincia"]);

		$regiones = array();		
		foreach($data["ciudades"] as $ciudad)
		{
			$regiones[$ciudad->id] = $ciudad->ciudad_nombre;			
		}
	...
	{	
		...
		// --
		// Verifico que el CUIT no esté duplicado
