...
class Banner extends CI_Controller {
	...
	public function index()
	{
		...
		$info =  $this->page_model->get_banners();
		$data['info']=$info;
	    ...
	    $this->template->write_view('content', 'layout/banner/list', $data, TRUE); 
	...
	public function add(){
		...
		$cat =  $this->page_model->get_active_provinces();
		$data['provincias']=$cat;
		
		$this->template->write_view('content', 'layout/banner/add', $data, TRUE); 
		$this->template->render();
	}
	...
	public function save(){
		if (isset($this->session->userdata['logged_in'])) {
			$data = array(
				...
				'titulo' => $_POST['titulo'],
				'link' => $_POST['link'],
				'link_target' => $_POST['linktarget'],
				'descripcion' => $_POST['descripcion'],
				'imagen_header' => $_POST['galeria1_input']
			);
			$this->page_model->insert_banner($data);
			redirect('banner/');
		}else{
			redirect('login/');
		}
	...
	public function update(){
		if (isset($this->session->userdata['logged_in'])) {
			...
			$this->page_model->update_banner($data);
			redirect('banner/');
		}else{
			redirect('login/');
		}
	...
	public function edit(){
		...
		$CI =& get_instance();
		$info =  $this->page_model->get_banner_id($this->uri->segment(3));
		$data['info']=$info;
		
		$cat =  $this->page_model->get_active_provinces();
		$data['provincias']=$cat;		
		
		$this->template->write_view('content', 'layout/banner/edit', $data, TRUE); 
		$this->template->render();
	}
