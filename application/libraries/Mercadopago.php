...
class Mercadopago
{
	...
	{
		if ($config['use_access_token'] === TRUE && ! empty($config['access_token'])) {
        	$this->ll_access_token = $config['access_token'];
		}
		else if (! empty($config['client_id']) && ! empty($config['client_secret'])) {
			$this->client_id = $config['client_id'];
			$this->client_secret = $config['client_secret'];
		}
		else {
			throw new MercadoPagoException("Invalid arguments. Set CLIENT_ID and CLIENT_SECRET, or ACCESS_TOKEN in Mercadopago config file.");
		}
		
		$this->sandbox = $config['sandbox_mode'] === TRUE;
	}
